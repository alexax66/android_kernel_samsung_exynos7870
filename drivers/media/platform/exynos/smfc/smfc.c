	...
	if (ctx) {
		...
		if (vb) {
			if (!!(ctx->flags & SMFC_CTX_COMPRESS)) {
				vb2_set_plane_payload(vb, 0, streamsize);
				if (!!(ctx->flags & SMFC_CTX_B2B_COMPRESS))
					vb2_set_plane_payload(vb, 1,
							thumb_streamsize);
			}

			vb->v4l2_buf.reserved2 =
		...
		void *alloc_ctxs[])
{
	...
	if (smfc_is_compressed_type(ctx, vq->type)) {
		...
		sizes[0] = PAGE_SIZE;
		*num_planes = 1;
		alloc_ctxs[0] = ctx->smfc->vb2_alloc_ctx;
		if (!!(ctx->flags & SMFC_CTX_B2B_COMPRESS)) {
			sizes[1] = PAGE_SIZE;
			alloc_ctxs[1] = ctx->smfc->vb2_alloc_ctx;
			(*num_planes)++;
		}
	} else {
		unsigned int i;
		*num_planes = ctx->img_fmt->num_buffers;
		for (i = 0; i < *num_planes; i++) {
			...
			alloc_ctxs[i] = ctx->smfc->vb2_alloc_ctx;
		}
		...
		if (!!(ctx->flags & SMFC_CTX_B2B_COMPRESS)) {
			...
			for (i = 0; i < *num_planes; i++) {
				...
				sizes[idx] /= 8;
				alloc_ctxs[idx] = ctx->smfc->vb2_alloc_ctx;
			}
...
static int smfc_vb2_buf_prepare(struct vb2_buffer *vb)
{
	...
	unsigned int i;
	bool clean_cache = !(vb->v4l2_buf.flags & V4L2_BUF_FLAG_NO_CACHE_CLEAN);
...
static int exynos_smfc_probe(struct platform_device *pdev)
{
	...
	if (!smfc->vb2_alloc_ctx) {
		dev_err(&pdev->dev, "Failed to allocate vb2-ion context");
		return -ENOMEM;
	}
